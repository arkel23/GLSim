...
def get_b16_config():
    ...
    config.sd = 0.0
    config.classifier_aux = 'cont'
    config.patchifier_dropout = False
    return config
...
class TFG(nn.Module):
    ...
    def forward(self, x):
        ...
        part_logits = part_logits.view(-1, self.num_classes)
        if self.classifier_aux == 'cont':
            return part_logits, part_tokens[:, 0]
        return part_logits
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            for bname, block in self.transformer.encoder.named_children():
                if bname.startswith('part') is False:
