...
def load_state_dict(model_name='B_16', map_location='cpu'):
    ...
    if not os.path.exists(cached_file_npz):
        sys.stderr.write(
            'Downloading: "{}" to {}\n'.format(URL, cached_file_npz))
        download_url_to_file(URL, cached_file_npz, progress=True)
    if _is_legacy_zip_format(cached_file_npz):
        return _legacy_zip_load(cached_file_npz, model_dir, map_location)
...
def get_b16_config():
    ...
    config.num_token = 12
    config.sd = 0.0
    config.patchifier_dropout = True
    return config
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            ...
            if self.feature_fusion:
                ...
                selected_num, selected_inx = self.ff_token_select(
                    weights, contribution)
                B = selected_inx.shape[0]
                for i in range(B):
                    tokens[i].extend(
                        hidden_states[i, selected_inx[i, :self.num_token]])
            # '''
            if self.vis:
        ...
        if self.feature_fusion:
            ...
            concat = torch.cat(
                (hidden_states[:, 0].unsqueeze(1), tokens), dim=1)
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                print("load_pretrained: resized variant: %s to %s" %
